...
{
    private const CODE = "ukvat";
    private const SCHEME_CURRENCY = 'GBP';
    private const array SCHEME_COUNTRIES = ['GB','IM'];
    private const array EU_COUNTRIES = ['AT','BE','BG','HR','CY','CZ','DK','EE','FI','FR','DE','GR','HU','IE',
     ...
     * @param HMRCConnector $hmrcConnector
     */
    public function __construct(
        ...
        HMRCConnector $hmrcConnector
    ) {
        $this->scopeConfig = $scopeConfig;
        $this->logger = $logger;
        $this->currencyFactory = $currencyFactory;
        $this->ticrFactory = $ticrFactory;
        $this->hmrcConnector = $hmrcConnector;
    }
     ...
     *
     * @param TaxIdCheckResponseInterface $taxIdCheckResponse
     * @param string|null $taxId
     * @return TaxIdCheckResponseInterface
     */
    private function validateFormat(
        TaxIdCheckResponseInterface $taxIdCheckResponse,
        ?string $taxId
    ): TaxIdCheckResponseInterface {
    ...
    private function validateOnline(
        ...
        try {
            $response = $this->hmrcConnector->getVATRegistrationWithReference(
            ...
            if (!$response->failed()) {
                ...
                $responseDto = $response->dto();
                if ($responseDto->code == "NOT_FOUND") {
                    $taxIdCheckResponse->setIsValid(false);
                    $taxIdCheckResponse->setRequestSuccess(true);
                    $taxIdCheckResponse->setRequestMessage(__('VAT Number could not be found at HMRC.'));
                } else {
                    $taxIdCheckResponse->setRequestSuccess(false);
                    $taxIdCheckResponse->setIsValid(false);
